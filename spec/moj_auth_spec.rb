...
describe 'RackMojAuth::Middleware' do
  ...
  it 'it bounces requests that have no HTTP_X_SECURE_TOKEN header' do
  ...
  it 'it bounces requests that have an invalid HTTP_X_SECURE_TOKEN header' do
    stub_request(:get, "#{@auth_service_url}/users/invalid.json")
      .to_return(status: 401, body: {error: 'Invalid token'}.to_json, headers: {})
    response = @backend.post('/any_url', {'HTTP_X_SECURE_TOKEN' => 'invalid'})

    expect(response.status).to eql 403
    expect(response.body).to be_empty
  end

  it 'it passes requests with a valid HTTP_X_SECURE_TOKEN header' do
    ...
    response = @backend.post('/any_url', {'HTTP_X_SECURE_TOKEN' => 'Pm2tbZfcwfD7B1jK_wzo'})

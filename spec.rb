...
describe 'moj_auth Middleware' do
  ...
  context 'Unauthenticated user' do
    ...
    describe '/register' do
      ...
      describe 'failure' do
        it 'returns 400 status code' do
          resp = ApiClient.post('/register', @bad_creds)
    ...
    describe '/login' do
      describe 'success' do
        it 'returns 200 status code' do
          resp = ApiClient.post('/login', @good_creds)
      ...
      describe 'failure' do
        it 'returns 403 status code' do
          resp = ApiClient.post('/login', @bad_creds)
    ...
    describe 'missing secure-token header' do
      it 'returns 403 status code' do
        resp = ApiClient.get('/valid_route_on_api')
        resp.code.should eq 403
      end
    ...
    describe 'invalid secure-token header' do
      it 'returns 403 status code' do
        resp = ApiClient.get('/valid_route_on_api', :headers => {'x-secure-token' => 'bad-token'})
        resp.code.should eq 403
      end
  ...
  context 'Authenticated user' do
    ...
    describe '/logout' do
      it 'returns 200 status code' do
        resp = ApiClient.post('/logout', :headers => @valid_secure_token_header)
        resp.code.should eq 200
      end
    end
    ...
    describe 'all other urls, with valid x-secure-token header' do
      it 'status code passed through from API' do
        resp = ApiClient.get('/valid_route_on_api', :headers => @valid_secure_token_header)
    ...
    describe 'roles enrichment' do
      ...
      it 'filters client-side x-user-id header' do
        resp = ApiClient.get('/valid_route_on_api', :headers => {'x-user-id' => 'auth_bypass'}.merge(@valid_secure_token_header))
        resp.body.should_not include('auth_bypass')
      end

      it 'filters client-side x-user-role header' do
        resp = ApiClient.get('/valid_route_on_api', :headers => {'x-user-role' => 'auth_bypass'}.merge(@valid_secure_token_header))
        resp.body.should_not include('auth_bypass')
      end
    end
  end
